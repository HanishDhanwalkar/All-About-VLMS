...
def verify_n_unzip_vqa():
    print("Find the VQA dataset from (https://visualqa.org/download.html)")
    
    if not os.path.exists('data/v2_Annotations_Train_mscoco.zip'):
        raise Exception('Please download VQA annotations.')
    if not os.path.exists('data/v2_Annotations_Val_mscoco.zip'):
        raise Exception('Please download VQA annotations.')

    if not os.path.exists('data/v2_Questions_Train_mscoco.zip'):
        raise Exception('Please download VQA questions.')
    if not os.path.exists('data/v2_Questions_Val_mscoco.zip'):
        raise Exception('Please download VQA questions.')
    if not os.path.exists('data/v2_Questions_Test_mscoco.zip'):
        raise Exception('Please download VQA questions.')
    
    print('Verified and unzipping VQA data...')
    os.system('unzip data/v2_Questions_Train_mscoco.zip -d data/')
    os.system('unzip data/v2_Questions_Val_mscoco.zip -d data/')
    os.system('unzip data/v2_Questions_Test_mscoco.zip -d data/')
    os.system('unzip data/v2_Annotations_Train_mscoco.zip -d data/')
    os.system('unzip data/v2_Annotations_Val_mscoco.zip -d data/')
    
    print('Unzipped VQA data.\n')

def main(params):
    '''
    # Download the dataset from https://visualqa.org/download.html
    ...
    print("Details of annotations and questions:")
    print(train_anno.keys())
    print(train_anno['annotations'][0].keys())
    print(train_ques.keys())
    print(train_ques['questions'][0].keys())
